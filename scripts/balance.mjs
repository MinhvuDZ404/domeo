...
// What the bot still wants before it can build and use its first campfire.
function wantedResource(game, random) {
...
function play(seed) {
  ...
  for (let step = 0; step < steps; step++) {
    ...
    const type = wantedResource(game, random);
    ...
    for (const recipe of RECIPES) {
      if (!['axe', 'pickaxe', 'campfire'].includes(recipe.id)) continue;
    ...
    mark('firstNight', game.elapsed >= DAY_LENGTH * 0.5);
    mark('day2', game.elapsed >= DAY_LENGTH);
    if (game.dead) {
      milestones.died = game.elapsed;
      break;
    }
    if (game.stats.campfires > 0 && game.elapsed >= DAY_LENGTH) break;
  }
...
function survive(seed) {
  ...
  return { seed, died: game.dead, elapsed: game.elapsed, berriesEaten, hunger: game.player.hunger };
}
