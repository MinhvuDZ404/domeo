...
// player_walk_new.png có nhãn điều khiển ở 128px đầu bên trái.
// Sau phần nhãn là 8 cột x 4 hàng: mỗi ô 206x224px.
const SPRITE_LABEL_WIDTH = 128;
...
const player = {
    ...
    direction: 0, frameIndex: 0, animTimer: 0, isMoving: false
};
...
function getChunkTrees(chunkX, chunkY) {
    ...
    for (let index = 0; index < treeCount; index++) {
        ...
        trees.push({ x, y, width: 64, height: 96,
...
function getVisibleTrees() {
    ...
    for (let chunkY = minChunkY; chunkY <= maxChunkY; chunkY++) {
        for (let chunkX = minChunkX; chunkX <= maxChunkX; chunkX++) trees.push(...getChunkTrees(chunkX, chunkY));
    }
    return trees;
}
function getHitbox(obj) {
    return { x: obj.x + obj.hitbox.offsetX, y: obj.y + obj.hitbox.offsetY,
        width: obj.hitbox.width, height: obj.hitbox.height };
}
...
function update(dt) {
    let dx = 0, dy = 0;
    if (keys['a'] || keys['arrowleft']) { dx--; player.direction = 1; }
    if (keys['d'] || keys['arrowright']) { dx++; player.direction = 2; }
    if (keys['w'] || keys['arrowup']) { dy--; player.direction = 3; }
    if (keys['s'] || keys['arrowdown']) { dy++; player.direction = 0; }
    ...
    if (dx !== 0) {
        ...
        player.x += movement;
        if (trees.some(tree => checkAABB(getHitbox(player), getHitbox(tree)))) player.x -= movement;
    }
    if (dy !== 0) {
        const movement = dy * PLAYER_SPEED * dt;
        player.y += movement;
        if (trees.some(tree => checkAABB(getHitbox(player), getHitbox(tree)))) player.y -= movement;
    }
    if (player.isMoving) {
        player.animTimer += dt * 1000;
        while (player.animTimer >= FRAME_DURATION) {
            player.frameIndex = (player.frameIndex + 1) % SPRITE_COLUMNS;
...
function render() {
    ...
    const renderList = [];
    getVisibleTrees().forEach(tree => renderList.push({
    ...
    renderList.push({
        ...
        draw: () => {
            ...
            if (images.player.loaded) {
                // Bỏ 128px nhãn bên trái; cắt đúng từng nhân vật theo hàng/cột.
                const sx = SPRITE_LABEL_WIDTH + player.frameIndex * SPRITE_FRAME_WIDTH;
                const sy = player.direction * SPRITE_FRAME_HEIGHT;
                ctx.drawImage(images.player.img, sx, sy, SPRITE_FRAME_WIDTH, SPRITE_FRAME_HEIGHT,
